...
namespace SubfileDrift {

const Int_t HIST_BINS = 400;
const Double_t HIST_MIN = 0.0;
const Double_t HIST_MAX = 10000.0;

// Same per-channel peak windows DiagnoseGainDrift uses; if the lowest-peak
// feature moves, those windows still bracket it.
...
inline PeakLoc WindowedPeak(TH1D *h, Double_t lo, Double_t hi) {
  ...
  p.mu_err = h->GetBinWidth(max_bin);
  p.ok = kTRUE;
  return p;
}
...
inline void FillSubfilePeaks(const TString &events_path,
  ...
  out_peaks.assign(chans.size(), {0, 0, kFALSE});
...
void DriftOneRun(Int_t run, const std::vector<FileSpec> &specs) {
  ...
  std::vector<std::vector<PeakLoc>> peaks(n_chans, std::vector<PeakLoc>(n_sub));
  ...
  for (Int_t i = 0; i < n_chans; i++) {
    ...
    for (Int_t s = 0; s < n_sub; s++) {
      ...
      Double_t ey = peaks[i][s].mu_err / mu_ref;
      g->SetPoint(pt, Double_t(s), y);
      g->SetPointError(pt, 0.0, ey);
      if (y - ey < y_min)
        y_min = y - ey;
      if (y + ey > y_max)
        y_max = y + ey;
  ...
  for (Int_t k = 0; k < Int_t(legend_channel_idx.size()); k++) {
    TGraphErrors *g = static_cast<TGraphErrors *>(graph_list->At(k));
...
void DiagnoseSubfileDrift() {
  ...
  for (Int_t r = 0; r < Int_t(Constants::RUN_NUMBERS.size()); r++) {
    ...
    std::cout << "Subfile drift for run " << run << " (" << run_specs.size()
              << " subfiles)" << std::endl;
    SubfileDrift::DriftOneRun(run, run_specs);
  }
}
