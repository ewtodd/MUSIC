...
void TraceCreator::BuildMeVSummaryHistograms(const TString &input_filename,
  ...
  TH2F *h2_cath = new TH2F("h2_totalEMeV_vs_cathodeADC",
                           ";Cathode #DeltaE [ADC];Total #DeltaE [MeV]", 200,
                           0.0, 16384.0, 400, total_e_min, total_e_max);
  ...
  for (Long64_t j = 0; j < n_entries; j++) {
    ...
    if (cathode_raw != -1)
      h2_cath->Fill(Double_t(cathode_raw), event_total);
  }

  {
    ...
    WriteH2CanvasToFile(input_file, h2_cath, "totalE_mev_vs_cathode_adc",
                        trace_subdir);
  }
