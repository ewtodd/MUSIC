...
int extraction_calib_coef(string file, Double_t centL[], Double_t sigL[],
                          Double_t centR[], Double_t sigR[]) {
  ifstream in;
  in.open(file.c_str());
  Int_t nlines = 0;
  while (1) {
    in >> centL[nlines] >> sigL[nlines] >> centR[nlines] >>
        sigR[nlines]; // MeV/a.u.
    if (!in.good())
      break;
    nlines++;
...
int extraction_inputs() {
  cout << pathRun << endl;
  string path_input = pathRun + "/inputs.dat";
  cout << path_input << endl;
  ...
  int nlist = 0;
  while (1) {
    if (nlines == 0 || nlines == 4 || nlines == 9 || nlines == 13 ||
        nlines == 20 || nlines == 23 || nlines == 28) {
    ...
    if (nlines == 16 || nlines == 22 || nlines == 26 || nlines == 27) {
      in >> index_inputs[nlines] >> variable[nlines] >>
          param_inputs[nlines][0] >> param_inputs[nlines][1];
      cout << index_inputs[nlines] << " " << variable[nlines] << " "
           << param_inputs[nlines][0] << " " << param_inputs[nlines][1] << endl;
    }
    if (nlines == 18 || nlines == 19) {
      in >> index_inputs[nlines] >> variable[nlines] >>
          param_inputs[nlines][0] >> param_inputs[nlines][1] >>
          param_inputs[nlines][2] >> param_inputs[nlines][3];
      cout << index_inputs[nlines] << " " << variable[nlines] << " "
           << param_inputs[nlines][0] << " " << param_inputs[nlines][1] << " "
           << param_inputs[nlines][2] << " " << param_inputs[nlines][3] << endl;
    }
    if (nlines != 0 && nlines != 4 && nlines != 6 && nlines != 7 &&
        nlines != 9 && nlines != 13 && nlines != 16 && nlines != 18 &&
        nlines != 19 && nlines != 20 && nlines != 22 && nlines != 23 &&
        nlines != 26 && nlines != 27 && nlines != 28) {
      in >> index_inputs[nlines] >> variable[nlines] >> param_inputs[nlines][0];
      cout << index_inputs[nlines] << " " << variable[nlines] << " "
           << param_inputs[nlines][0] << endl;
    }
    if (!in.good())
      break;
    nlines++;
...
int extraction_ShiftBoardTime(int RunNumber, Double_t shiftTime[NBoard]) {
  int find = 0;
  int Run_list[MaxN];
  Double_t Shift_list[MaxN][NBoard];
  string pathSys = gSystem->pwd();
  string file = pathRun + "/build/ShiftBoardTime.dat";
  cout << file << endl;
  ifstream in;
  in.open(file.c_str());
  Int_t nlines = 0;
  while (1) {
    in >> Run_list[nlines] >> Shift_list[nlines][0] >> Shift_list[nlines][1] >>
        Shift_list[nlines][2] >> Shift_list[nlines][3]; // sec
    if (!in.good())
      break;
    nlines++;
  }
