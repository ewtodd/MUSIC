...
private:
  ...
  template <typename T>
  int FillData(
      T *dataItem); // if successful, return 1, else 0; cannot fill Trace
...
public:
  ...
  void
  SetHeader(unsigned short header); /// when no header, this has to be set, 4bit
...
void BinReader::SetHeader(unsigned short header) { data.Header = header; }
...
int BinReader::ReadBlock(int opt) {
  ...
  if ((data.Header & 0x8) == 1) {
    ...
    if (opt == 0) {
      if (fread(data.Trace, sizeof(data.Trace), 1, inFile) != 1)
        endOfFile = IsEndOfFile();
    } else { /// skip trace
      fseek(inFile, inFilePos + data.NSample * 2,
            SEEK_SET); /// 2 becasue each trace is 2 bytes
    }
  }
...
void BinReader::PrintStatus(int mod) {
  ...
  if (blockID % mod == 0) {
    ...
    gClock.Start("timer");
    printf(
        "Total measurements: \x1B[32m%llu \x1B[0m\nReading Pos: \x1B[32m %.3f/%.3f GB\x1B[0m\nTime used:%3.0f min %5.2f sec\033[A\033[A\r",
        blockID, inFilePos / (1024. * 1024. * 1024.),
        inFileSize / 1024. / 1024. / 1024, TMath::Floor(time / 60.),
