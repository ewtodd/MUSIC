...
void computeNSD2(TGraph *ref, TGraph *gr, Double_t threshDT /*microsec*/,
                 Double_t OverlapTMin /*sec*/, Double_t OverlapTMax /*sec*/,
                 Int_t &npts, Double_t &NSD2) {
  NSD2 = 0;
  npts = 0;
...
Double_t findshift(TGraph *ref, TGraph *gr, Double_t OverlapTMin /*sec*/,
  ...
  for (Int_t p = 0; p < gr->GetN(); p++) {
    ...
    if (tmin < OverlapTMin && tmax > OverlapTMax) {
      ...
      computeNSD2(ref, grShifted, threshDT, OverlapTMin, OverlapTMax, npts,
                  NSD2);
      // cout << p << " ";
      // cout.precision(10);
      // cout << shift;
