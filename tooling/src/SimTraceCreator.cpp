...
void SimTraceCreator::BuildTraces(std::vector<TString> input_output_filepaths,
                                  std::vector<TString> file_labels,
                                  Bool_t save_plots, Bool_t reprocess) {
  if (!reprocess)
    return;

  Int_t n_files = input_output_filepaths.size();
  for (Int_t i = 0; i < n_files; i++) {
    TString input_output_filepath = input_output_filepaths[i];
    ...
    for (Int_t s = s_lo; s <= s_hi; s++) {
      h2_TotalE_vs_StripE[s] =
          new TH2D(Form("h2_TotalE_vs_StripE_s%d", s),
                   ...
                   Remix::STRIP_E_MIN_MEV, Remix::STRIP_E_MAX_MEV, 400,
                   Remix::TOTAL_E_MIN_MEV, Remix::TOTAL_E_MAX_MEV);
    }
    ...
    for (Long64_t j = 0; j < n_entries; j++) {
      ...
      if (save_plots && save_count < Remix::MAX_TRACE_SAVES) {
        save_count++;
        ...
        for (Int_t k = 0; k < n_active; k++) {
          ...
          if (Remix::IGNORE_SHORT_STRIPS) {
            if (Remix::IsShortSide(s, 'L'))
              l = 0.0;
            if (Remix::IsShortSide(s, 'R'))
              r = 0.0;
          }
        ...
        TraceTotal->GetYaxis()->SetRangeUser(Remix::STRIP_E_MIN_MEV,
                                             Remix::STRIP_E_MAX_MEV);
    ...
    input_output_file->Close();
    std::cout << "Finished processing " << input_output_filepath << std::endl;
  }
}

void SimTraceCreator::Run() {
  Bool_t reprocess_initial = kTRUE;

  std::vector<TString> input_output_filepaths, file_labels;

  std::vector<RemixSim::SimFileSpec> specs = RemixSim::BuildFileSpecs();
  for (Int_t k = 0; k < Int_t(specs.size()); k++) {
    input_output_filepaths.push_back(RemixSim::SimRootPath(specs[k]));
    std::cout << "Processing file: " << input_output_filepaths.back()
              << std::endl;
    file_labels.push_back(RemixSim::FileLabel(specs[k]));
  }

  const TString project_root = Paths::DatasetDir();
  InitUtils::SetROOTPreferences(PlotSaveFormat::kPNG, project_root + "/plots",
                                project_root + "/root_files");
  BuildTraces(input_output_filepaths, file_labels, kTRUE, reprocess_initial);
}
