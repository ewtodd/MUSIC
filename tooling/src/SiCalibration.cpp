...
void SiCalibration::Run() {
  ...
  if (!fit_file || fit_file->IsZombie()) {
    std::cerr << "ERROR: cannot open " << SiCalib::FIT_RESULTS_FILE
  ...
  TFile *mulgin_file = IO::OpenForReading(SiCalib::MULGIN_CALIBRATION_FILE);
  if (!mulgin_file || mulgin_file->IsZombie()) {
    std::cerr << "ERROR: cannot open " << SiCalib::MULGIN_CALIBRATION_FILE
  ...
  if (!mulgin_tree || mulgin_tree->GetEntries() < 1) {
    std::cerr << "ERROR: tree " << mulgin_tree_name << " missing or empty in "
              << SiCalib::MULGIN_CALIBRATION_FILE << std::endl;
    mulgin_file->Close();
    delete mulgin_file;
    fit_file->Close();
    delete fit_file;
  ...
  for (size_t i = 0; i < SiCalib::RUN_NUMBERS.size(); i++) {
    ...
    if (fits.count(run) == 0)
      continue;
    Bool_t group_A = run <= SiCalib::GAIN_GROUP_BOUNDARY_RUN;
    Double_t B = group_A ? B_A : B_B;
    Double_t C = group_A ? C_A : C_B;
    Double_t adc = fits[run].mu;
    energy_mev[run] = Calibration::MulginInverseEnergy(adc, B, C);
    energy_err_mev[run] =
        Calibration::MulginInverseEnergyError(adc, B, C, fits[run].mu_err);
  }
  ...
  for (size_t i = 0; i < SiCalib::RUN_NUMBERS.size(); i++) {
    Int_t run = SiCalib::RUN_NUMBERS[i];
    if (SiCalib::GAS_PRESSURE_TORR.count(run) == 0)
      continue;
    if (energy_mev.count(run) == 0)
      continue;
    Double_t dE = SiCalib::BEAM_ENERGY_MEV - energy_mev[run];
    Double_t dE_err =
        TMath::Sqrt(TMath::Power(SiCalib::BEAM_ENERGY_ERR_MEV, 2) +
  ...
  for (size_t i = 0; i < SiCalib::RUN_NUMBERS.size(); i++) {
    ...
    if (dE_mev.count(run) == 0)
      continue;
    pressures.push_back(SiCalib::GAS_PRESSURE_TORR.at(run));
  ...
  TFile *out_file = IO::OpenForWriting(SiCalib::CALIBRATION_RESULTS_FILE);
  ...
  for (size_t i = 0; i < SiCalib::RUN_NUMBERS.size(); i++) {
    ...
    if (fits.count(run) == 0)
      continue;
    run_number = run;
    gas_pressure_torr = SiCalib::GAS_PRESSURE_TORR.at(run);
