 ...
 * written beside the ROOT files as a small additional file and read back when
 * needed.
 */
class BinaryToRoot {
public:
  /// @brief Header filename for a run.
  /// @param run Run number.
  /// @return The bare filename, `DataR_run_<run>.header`, with no directory.
  static TString HeaderFileName(Int_t run);

  /**
   * @brief Write a run's global header to its file.
   *
   ...
   *
   * @note Failures to open the file are silent. The file is just a copy of what
   * was in the binary; a missing one is recoverable by re-reading the binary.
   */
  static void WriteHeaderFile(Int_t run, UShort_t header);

  /**
   * @brief Read a run's global header back from its file.
   *
   * @param run    Run number.
   ...
   *                    is honoured.
   *
   * @return `kTRUE` if the file exists and held a value; `kFALSE` if it is
   *         missing or empty.
   */
  static Bool_t ReadHeaderFile(Int_t run, UShort_t &header);
};
